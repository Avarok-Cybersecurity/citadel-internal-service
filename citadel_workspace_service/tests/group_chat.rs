...
mod tests {
    ...
    use citadel_workspace_types::{
        ...
        GroupListGroupsForSuccess, GroupRequestJoinAccepted, GroupRequestJoinFailure,
        GroupRequestJoinSuccess, GroupRespondInviteRequestFailure,
    ...
    async fn test_citadel_workspace_service_group_invite_and_join() -> Result<(), Box<dyn Error>> {
        ...
        }) = &deserialized_service_a_payload_response
        {
            ...
            }) = &service_c_group_inbound
            {
                ...
                // let service_c_group_inbound = from_service_c.recv().await.unwrap();
                // if let InternalServiceResponse::GroupListGroupsResponse(
                //     GroupListGroupsResponse {
                //         cid,
                //         group_list: actual_group_list,
                //         request_id
                //     },
                // ) = &service_c_group_inbound {
                //     info!(target: "citadel","Service C Response Showed {actual_group_list:?} for Service A Groups");
                // } else {
                //     panic!("Service C did not received Requested Group List");
                // }
                // info!(target: "citadel","LISTED GROUPS: {group_list:?}");
                if let Some(&group_to_join) = group_list.clone().unwrap().first() {
                    info!(target: "citadel","Service C Found {group_to_join:?} for Service A Groups");
